...
export function ApiKeyCard({
  ...
  const handleGenerate = async () => {
    if (
      displayKey &&
      !confirm(
        "Rotating your key will immediately invalidate existing log drains and ingestion scripts. Continue?",
      )
    ) {
      return;
    }
  ...
  return (
    <Card className="border-border/60 bg-card/40 backdrop-blur">
      <CardHeader>
        ...
        <CardDescription>
          Authenticate log forwarders (e.g. Vercel Log Drains) sending errors to{" "}
          <code>/api/telemetry/ingest</code>.
        </CardDescription>
      </CardHeader>
        ...
        {revealedRawKey && (
          <div className="p-3 rounded-md bg-amber-500/10 border border-amber-500/20 text-xs text-amber-300 space-y-2">
            <div className="flex items-center gap-1.5 font-semibold">
              <AlertTriangle className="h-4 w-4" /> Copy your new secret key
              now:
            </div>
            <div className="flex items-center gap-2">
              <Input
              ...
              <Button
                size="sm"
                onClick={() => handleCopy(revealedRawKey)}
                className="gap-1"
              >
                {copied ? (
            ...
            <p className="text-[11px] text-amber-400/80">
              For security, this raw key will never be shown again once
              refreshed.
            </p>
          </div>
        )}
