...
function CodeBlock({ children, className, ...props }: any) {
 ...
 return (
 <code
 className="px-1.5 py-0.5 mx-0.5 bg-zinc-900 border border-zinc-800 text-zinc-200 font-mono text-[11px] rounded-sm select-all"
 {...props}
 >
 {children}
 </code>
 ...
 return (
 <div className="relative my-3 rounded-xl border border-zinc-800/90 bg-zinc-950 overflow-hidden group">
 ...
 <Terminal className="h-3 w-3 text-zinc-200" />
 {lang || "code"}
 </span>
 <button
 type="button"
 onClick={handleCopy}
 className="flex items-center gap-1 px-1.5 py-0.5 text-[9px] hover:text-white text-zinc-400 bg-zinc-800/40 hover:bg-zinc-800 border border-zinc-700/50 transition-colors rounded-xl cursor-pointer"
 >
 {copied ? (
 <>
...
export function MarkdownRenderer({ content, className = "" }: MarkdownRendererProps) {
 ...
 <span className="text-zinc-200">#</span> {children}
 </h1>
 ),
 h2: ({ children }) => (
 ...
 ul: ({ children }) => (
 <ul className="my-2 pl-4 space-y-1 list-disc marker:text-white/70 text-zinc-300 text-[11px]">
 {children}
 </ul>
 ),
 ol: ({ children }) => (
 <ol className="my-2 pl-4 space-y-1 list-decimal marker:text-zinc-200 font-mono text-[11px] text-zinc-300">
 {children}
 </ol>
 ),
 li: ({ children }) => (
 ...
 blockquote: ({ children }) => (
 <blockquote className="my-2.5 pl-3 py-1 border-l-2 border-zinc-700/70 bg-white-950/15 text-zinc-400 font-sans text-[11px] italic">
 {children}
 </blockquote>
 ),
 table: ({ children }) => (
 <div className="my-3 overflow-x-auto border border-zinc-800 bg-zinc-950/60 rounded-xl scrollbar-thin">
 ...
 className="text-zinc-200 hover:text-white-300 underline underline-offset-2 transition-colors cursor-pointer"
 >
 {children}
 </a>
 ),
