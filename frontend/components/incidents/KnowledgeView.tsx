...
export function KnowledgeView({ organizationId }: KnowledgeViewProps) {
 ...
 <label className="flex flex-col items-center justify-center p-8 border border-dashed border-zinc-800 bg-zinc-950/50 hover:bg-zinc-900 hover:border-zinc-700 transition-colors cursor-pointer group rounded-xl">
 {isUploading ? (
 <Loader2 className="h-6 w-6 text-zinc-200 animate-spin mb-3" />
 ) : (
 <Upload className="h-6 w-6 text-zinc-500 group-hover:text-zinc-200 mb-3 transition-colors" />
 )}
 ...
 <FileText className="h-4 w-4 text-white/70 mt-0.5 shrink-0" />
 ...
 <span className="px-1.5 py-0.5 bg-zinc-900 rounded-xl uppercase">{doc.type}</span>
